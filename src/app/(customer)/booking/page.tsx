...
export default function CustomerBookingPage() {
    ...
    const { data: senderAddresses, mutate: mutateSenderAddresses } = useApi<SavedAddress[]>(session ? '/api/employee/addresses?type=sender' : null);
    const { data: receiverAddresses, mutate: mutateReceiverAddresses } = useApi<SavedAddress[]>(session ? '/api/employee/addresses?type=receiver' : null);
    ...
    const onSubmit = async (values: ShipmentBookingFormValues) => {
        ...
        try {
            ...
            const result = await response.json();
            if(response.ok) {
                toast({ title: "Booking Initiated", description: `Shipment ${result.shipment_id_str} is pending payment.` });
                form.reset();
                setPriceDetails(null);
                // Ideally, redirect to a payment page for this shipment
    ...
    return (
        <div>
        <h1 className="text-2xl font-bold mb-6">Create a New Shipment</h1>
        <Form {...form}>
                        ...
                        <Card>
                            <CardHeader className="flex flex-row items-center justify-between">
                                <div>
                                    ...
                                    <CardDescription>Where is the shipment coming from?</CardDescription>
                                </div>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                {priceDetails && !isCalculating && (
                                    <div className="space-y-2 text-sm">
                                        ...
                                        <div className="flex justify-between font-bold text-lg">
                                         ...
                                         <p className="text-xs text-muted-foreground pt-2">This price is inclusive of all taxes and fees. Proceed to book and complete the payment.</p>
                                    </div>
                                )}
                                {!priceDetails && !isCalculating && (
                             ...
                             <CardContent>
                                <Button type="submit" size="lg" className="w-full h-12 text-lg" disabled={!priceDetails || isSubmitting || isCalculating}>
                                    ...
                                    {isSubmitting ? "Booking..." : "Proceed to Book"}
                                </Button>
                             </CardContent>
                        </Card>
                    </div>
